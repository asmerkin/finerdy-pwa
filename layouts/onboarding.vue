...
const availableLocales = computed(() =>
  locales.value.filter((l): l is { code: 'en' | 'es'; name: string } =>
...
const switchLocale = (event: Event) => {
  ...
  if (selectedLocale === 'en' || selectedLocale === 'es') {
...
<template>
  <div class="flex min-h-screen flex-col items-center bg-gray-100 pt-6 antialiased sm:justify-center sm:pt-0">
    ...
    <!-- Content -->
    <div class="mt-6 w-full overflow-hidden bg-white px-6 py-4 shadow-md sm:max-w-xl sm:rounded-lg">
      <slot />
    </div>
  </div>
</template>
